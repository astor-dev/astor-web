...
author: "Astor"
title: "Java Socket 기반 HTTP 서버 구현, 서블릿(Servlet) API의 필요성과 개념"
pinned: true
draft: false
tags:
  - HTTP
...
직접 원시적인 HTTP를 구현해보니 정말 너무나도 자유롭습니다.
TCP 소켓 위에 데이터를 얹고, 응용 프로그램에서 그것을 해석하고 의미를 부여하면 그게 곧 HTTP가 됩니다. JDK 자체는 `Socket` 클래스까지만 지원하지, 그 위의 HTTP 같은 응용 프로토콜은 제공하지 않습니다.

즉, **HTTP라는 프로토콜의 모든 규칙을 준수하고 처리하는 책임이 전적으로 애플리케이션 개발자가 지게**됩니다.

- 요청 라인의 문법을 보장하는 사람도,
- 헤더 파싱을 정확히 수행하는 사람도,
- Content-Length나 Keep-Alive를 제대로 처리하는 사람도,

순수한 Java 사이드에는 아무것도 없습니다. 직접 구현한 코드가 곧 사실상의 “규칙”이 되어버립니다. 작성한 코드가 **HTTP/1.1의 표준 명세를 엄밀하게 지킨다**고 보장할 수가 없습니다.

그러면 **Java 애플리케이션에서 무언가가 이 응용 프로토콜을 올바르게 준수하도록 보장해야** 하지 않을까요? 바로 **규격화된 명세**가 필요해집니다.
