...
런타임에 유실되지 않기 때문에 모듈 선언부나 사용부의 코드가 굉장히 심플해집니다. 개인적으로는 ts에선 위와 같은 추상 클래스 패턴 사용을 꽤나 권장하는 편인데, 주로 **설계도 : 구현체가 1:1 대응될 확률이 높은 경우** 위 패턴 사용을 추천합니다. java로 치면 service -> serviceImpl 혹은 DefaultService 만 존재하는 케이스에 속하죠.
...
```ts
  constructor(
    @Inject(TODO_FIND) private readonly finder: TodoFind,
...
여기까지 delegation을 통해 **여러 역할에 하나의 구현체를 주입**하는 방법을 다뤘습니다.
