...
이후 플로우는 **프로퍼티 리졸브** ➡ **생성자 리졸브** ➡ **클래스의 인스턴스 생성(콜백)**&#xC73C;로 이어집니다.
...
이 리졸브 개념과 스코프 개념을 이해하시고 나면 아래의 질문에 대답할 수 있을 것입니다.

> **"싱글톤 인스턴스는 한 번만 만들어지는데, 그 안에 있는 Request 스코프 의존성은 어떻게 매 요청마다 다르게 들어가는 걸까?"**

**A) 싱글톤 인스턴스 내부에 주입된 Request 스코프 의존성**도, 싱글톤이 생성된 후에도 실제 사용 시점에 "현재 요청"의 contextId에 따라 동적으로 resolve되어 매 요청마다 다른 인스턴스가 주입되는 것이 가능하다.
...
- **CatsService**
  가 Request 스코프로 설정되어 있다면, 이를 주입받는 
  **CatsController**
  도 Request 스코프로 전환됩니다.
- 반면, 
  **CatsRepository**
...
- Transient로 지정된 의존성은 매번 새로운 인스턴스가 생성됩니다. 단, 이 경우에도
   
  **의존성 주입 체인 전체가 Transient가 되는 것은 아닙니다.**
- DogsService는 여전히 Singleton으로 유지됩니다.
- DogsService 내부에서 LoggerService를 사용할 때마다 매번 새로운 LoggerService 인스턴스가 생성되어 주입됩니다.
