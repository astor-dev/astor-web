...
author: "Astor"
title: "NestJS DI는 어떻게 작동할까? 프레임워크 내부 코드로 직접 파헤쳐보기"
pinned: true
draft: false
tags:
  - NestJS
...
Token은 Provider를 주입할 때 식별하기 위한 일종의 식별자입니다. **사용가능한 타입들을 잘 봐두시기 바랍니다**. 위를 통해 DIP 등 여러 테크닉을 구사할 수 있습니다.

바로 보겠습니다.

Custom Provider과 Symbol을 활용한 Java Spring 스타일의 **interface 기반 DIP** 예제입니다.
...
가져오는 쪽 코드를 보면,  `const instancePerContext = this.values.get(contextId)`이 부분에서 contextId 기반으로 인스턴스를 가져오는데, 싱글톤의 경우 STATIC\_CONTEXT라는 별도의 전용 contextId가 존재해서 같은 로직으로 Request Scope와 Default Scope를 같이 처리합니다.
...
또한 InstanceWrapper의 프로퍼트 부분을 보면 WeakMap으로 contextId에 해당하는 인스턴스를 저장해놓고 있습니다. STATIC\_CONTEXT에 해당하는 인스턴스 또한 이렇게 싱글톤으로 관리되고 있는 걸 알 수 있습니다. 마찬가지로 RequestScope 별 1개의 인스턴스를 유지하는 방식도 Wrapper를 통해 Map으로 관리되기에 가능하였습니다.
