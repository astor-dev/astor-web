...
위와 같은 기능을 수행합니다. 도출할 수 있는 인사이트가 되게 많은데, 제일 초반 부분에 스코프와 관련된 핵심 처리가 등장합니다.
...
NestJS는 스코프에 따라 인스턴스를 보관하는 context를 다르게 유지하며,
Singleton 인스턴스가 이미 존재해도, 내부 의존성은 매 요청마다 해당 context에 맞게 resolve되는 구조를 사용한다.
이를 통해 Nest는 스코프 간 주입이 유연하면서도 안전하게 이뤄지도록 보장한다.
